...
# Чтобы создать словарь из двух списков с равным количеством элементов, где первый список — это ключи, а второй — значения, можно использовать функцию zip() вместе с dict(). Вот пример:
...
# допустим хотим использовать элементы списка в качестве ключей, а применяя к.л. выражение к этим же элементам записывать результат в качестве ЗНАЧЕНИЙ: в общем виде:
'''
#  имя_cлoвapя = {переменная:выражение(переменная) for переменная in numbers}
'''
# -где переменная(при каждой итерации) принимает значения из списка numbers.
